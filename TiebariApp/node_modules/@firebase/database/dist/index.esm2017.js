 ...
 */
const resolveDeferredValue = function (value, existing, serverValues) {
 ...
 */
const resolveDeferredValueTree = function (tree, syncTree, serverValues) {
    const resolvedTree = new SparseSnapshotTree();
    tree.forEachTree(new Path(''), function (path, node) {
        const existing = syncTree.calcCompleteEventCache(path);
        assert(existing !== null && typeof existing !== 'undefined', 'Expected ChildrenNode.EMPTY_NODE for nulls');
...
const resolveDeferredValueSnapshot = function (node, existing, serverValues) {
    ...
    if (node.isLeafNode()) {
        ...
        childrenNode.forEachChild(PRIORITY_INDEX, function (childName, childNode) {
            const newChildNode = resolveDeferredValueSnapshot(childNode, existing.getImmediateChild(childName), serverValues);
...
class Repo {
    ...
    update(path, childrenToMerge, onComplete) {
        ...
        each(childrenToMerge, (changedKey, changedValue) => {
            ...
            changedChildren[changedKey] = resolveDeferredValueSnapshot(newNodeUnresolved, this.serverSyncTree_.calcCompleteEventCache(path), serverValues);
        });
        if (!empty) {
            const writeId = this.getNextWriteId_();
    ...
    runOnDisconnectEvents_() {
        ...
        const serverValues = this.generateServerValues();
        const resolvedOnDisconnectTree = resolveDeferredValueTree(this.onDisconnect_, this.serverSyncTree_, serverValues);
        let events = [];
